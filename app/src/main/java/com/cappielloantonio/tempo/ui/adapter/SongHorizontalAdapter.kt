...
class SongHorizontalAdapter(
    ...
    override fun onBindViewHolder(
        ...
        holder.item.searchResultSongSubtitleTextView.text =
        ...
        holder.item.trackNumberTextView.text =
        ...
        if (!showCoverArt &&
            (
                position == 0 ||
                    (
                        position > 0 && songs.get(position - 1) != null && songs.get(position - 1).discNumber != null && songs
                            .get(
                                position,
                            ).discNumber != null && songs.get(position - 1).discNumber!! <
                            songs
                                .get(
                                    position,
                                ).discNumber!!
            ...
            if (songs.get(position).discNumber != null &&
                !Objects
                    .requireNonNull<Int?>(
                        songs
                            .get(
                ...
                holder.item.discTitleTextView.text =
                    holder.itemView.context.getString(
            ...
            if (album.discTitles != null) {
                ...
                if (discTitle.isPresent && discTitle.get().disc != null && discTitle.get().title != null &&
                    !discTitle.get().title!!.isEmpty()
                ) {
                    holder.item.discTitleTextView.text =
                        holder.itemView.context.getString(
    ...
    inner class ViewHolder internal constructor(
        var item: ItemHorizontalTrackBinding,
    ...
    fun sort(order: String) {
        when (order) {
            ...
            Constants.MEDIA_MOST_RECENTLY_STARRED ->
                songs.sort(
                    Comparator.comparing<Child?, Date?>(
                        Child::starred,
                        Comparator.nullsLast<Date?>(Comparator.reverseOrder<Date?>()),
                    ),
                )

            Constants.MEDIA_LEAST_RECENTLY_STARRED ->
                songs.sort(
                    Comparator.comparing<Child?, Date?>(
                        Child::starred,
                        Comparator.nullsLast<Date?>(Comparator.naturalOrder<Date?>()),
                    ),
                )
        }
